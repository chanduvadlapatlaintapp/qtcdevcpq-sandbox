...
setSegmentedBundleKey(quoteLineModels);populateBundleDetails(quoteLineModels,conn);calculateServiceGrossTotal(quoteModel,quoteLineModels);// Instead of calling meterTypeCalculation multiple times, build a filtered list first.
...
return meterTypeCalculation(quoteModel,applicableQuoteLines,conn).then(function(result){// If everything passes, simply return the result.
return result;}).catch(function(error){console.error("Error in onAfterCalculate:",error);return Promise.reject(error);});}/**
 * @description
