...
export function onAfterCalculate(quoteModel, quoteLineModels, conn) {
    ...
    // Determine if this calculate was triggered by the AgenticQTC server-side flow.
    // Standard QLE / record Calculate / batch never set this flag → unchanged behavior.
    var __isAgentic = quoteModel && quoteModel.record &&
                      quoteModel.record.Created_by_Simplified_Quoting__c === true;

    // Called once (same as legacy). For AgenticQTC we await it via Promise.all so the
    // orphan query completes before resolution, eliminating the JSQC serialization race
    // ("Error parsing quote: Unexpected token ["). For all other callers the existing
    // fire-and-forget behavior is preserved exactly.
    var __bundlePromise = populateBundleDetails(quoteLineModels, conn);
    if (!__isAgentic) { __bundlePromise = null; }   // non-AgenticQTC → legacy, untouched
    ...
    var __calcPromise = meterTypeCalculation(quoteModel, applicableQuoteLines, conn)
        .then(result => {
    ...
    if (__bundlePromise) {
        // AgenticQTC path: await bundle work alongside calc so the async query
        // always completes before the quote is serialized back to SBQQ.JSQCClient.
        return Promise.all([ __bundlePromise.catch(e => console.log(e)), __calcPromise ]).then(r => r[1]);
    }

    return __calcPromise;
}
