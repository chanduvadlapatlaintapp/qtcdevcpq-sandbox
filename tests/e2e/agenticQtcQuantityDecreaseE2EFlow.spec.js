...
/** @param {number} v */
const applyDelta = v => Math.max(QTY_FLOOR, v + QTY_DELTA);
...
/**
 * Fill qty with applyDelta for each non-segment input.
 * @param {import('@playwright/test').Page} page
 * @param {any[]} products
 */
async function editNonSegInputs(page, products) {
  ...
  for (const p of products) {
    ...
    const newVal  = applyDelta(current);
...
async function editMdqRegularSegments(page, rows) {
  ...
  const expectedA = beforeA.map(v => applyDelta(v));
  ...
  await setSegmentQuantity(productA.row, 0, applyDelta(beforeA[0]));
  await expect.poll(() => readSegmentQuantities(productA.row), {
    message: `MDQ regular Product A: all ${productA.segmentCount} segments should reflect delta (floor ${QTY_FLOOR})`,
  ...
  expectedB[idxB] = applyDelta(beforeB[idxB]);

  await setSegmentQuantity(productB.row, idxB, applyDelta(beforeB[idxB]));
...
async function editMdqBundleSegments(page, rows) {
  ...
  await setSegmentQuantity(bundle.row, 0, applyDelta(beforeA[0]));
  await expect.poll(() => readSegmentQuantities(bundle.row), {
    message: `MDQ bundle: all ${bundle.segmentCount} segments should reflect delta (floor ${QTY_FLOOR})`,
    timeout: 10_000, intervals: [200, 400, 600, 800, 1000],
  }).toEqual(expectedA);
  ...
  await setSegmentQuantity(bundle.row, idxB, applyDelta(beforeB[idxB]));
  await page.waitForTimeout(QUIESCE_MS);
...
async function runScenario(page, contract, branch, scenarioNumber, scenarioLabel) {
  ...
  for (let i = 0; i < nsRegEdits.length; i++) {
    ...
    const expected = applyDelta(item.initial);
  ...
  buildRichResults({
    kind: 'quantityDecreaseE2EFlow', runTs, runDir, testStartMs,
    ...
    extra: {
      dbHeader, dbError, floor: QTY_FLOOR,
      ...
      mdqRegState: mdqRegState ? {
        productA: { name: mdqRegState.productA.productName, before: mdqRegState.beforeA, expected: mdqRegState.finalExpectedA, rule: 'Year 1 decrease propagates' },
        productB: { name: mdqRegState.productB.productName, before: mdqRegState.beforeB, expected: mdqRegState.expectedB, rule: `Year ${mdqRegState.idxB + 1} decrease isolated` },
      } : null,
      mdqBunState: mdqBunState ? {
        product: mdqBunState.bundle.productName,
        propagation: { before: mdqBunState.beforeA, expected: mdqBunState.expectedA, rule: 'Year 1 decrease propagates' },
        isolation:   { before: mdqBunState.beforeB, expected: mdqBunState.expectedB, rule: `Year ${mdqBunState.idxB + 1} decrease isolated` },
      } : null,
    },
  });
...
test('Scenario 1: contract with 0 amendments — fresh amendment, E2E quantity decrease', async ({ page }) => {
...
test('Scenario 2: contract with 1 amendment — existing draft, E2E quantity decrease', async ({ page }) => {
...
test('Scenario 3: contract with multiple amendments — modal pick, E2E quantity decrease', async ({ page }) => {
