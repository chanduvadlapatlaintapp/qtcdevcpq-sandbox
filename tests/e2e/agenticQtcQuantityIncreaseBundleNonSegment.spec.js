...
const KIND              = 'quantityIncreaseBundleNonSegment';
 ...
 * segment rows). Then query the DB to keep only those whose line ID has
 * CPQ_Option_Type__c = 'Static Bundle'.
 *
 * @param {import('@playwright/test').Page} page
 * @param {import('./utils/agenticQtcPage').AgenticQtcPage} qtc
 * @returns {Promise<Array<{rowIndex:number, row:import('@playwright/test').Locator, input:import('@playwright/test').Locator, lineId:string, initial:number, rowText:string, costBefore:string}>>}
 */
async function findNonSegmentBundleInputs(page, qtc) {
  ...
  for (let i = 0; i < rowCount; i++) {
    ...
    candidates.push({ rowIndex: i, row, input, lineId, initial: parseFloat(val) || 0, rowText, costBefore });
  }

  // Secondary: spinbuttons outside any tr.product-row
  ...
  for (let i = 0; i < sbCount; i++) {
    ...
    candidates.push({ rowIndex: -1, row, input: sb, lineId, initial: parseFloat(val) || 0, rowText, costBefore });
  }
 ...
 * }} ctx
 */
async function runQuantityIncreaseBundleNonSegment(ctx) {
  ...
  const bundleLines = await findNonSegmentBundleInputs(page, qtc);
  ...
  console.log(`[Scenario ${scenarioNumber}] Found ${bundleLines.length} non-segment bundle line(s) in quote ${quoteName}`);

  const metricsBeforeSave = await captureMetrics(page);

  /** @type {Array<typeof bundleLines[number] & { filled: number }>} */
  const preSave = bundleLines.map(l => ({ ...l, filled: 0 }));
  ...
  await u.screenshot(page, runDir, '03-after-qty-increase');
  ...
  for (let i = 0; i < preSave.length; i++) {
    ...
    if (dbQty > 0 && dbPrice !== null && dbPrice === 0 && dbListPrice && dbListPrice > 0) {
      dbAnomalies.push({ type: 'POSSIBLE 100% DISCOUNT', severity: 'HIGH',
        detail: `Bundle line "${db?.SBQQ__ProductName__c}": ListPrice=$${dbListPrice} but CustomerPrice=$0.` });
    }
  ...
  // ── DB: component lines must mirror each bundle's qty ────────────────────
  if (preSave.length > 0) {
    const expectedQty = preSave[0].filled;
    const compSoql = `SELECT Id, SBQQ__Quantity__c, SBQQ__ProductCode__c, SBQQ__ProductName__c,
                             CPQ_Option_Type__c, Revenue_Allocation__c, Revenue_Allocated__c
    ...
    if (comps.length > 0) {
      ...
      for (const c of comps) {
        ...
        dbComparison.push({
          ...
          priorQty: null, dbQty: Number(c.SBQQ__Quantity__c),
      ...
      console.log(`[Scenario ${scenarioNumber}] No non-segment Static Component lines found for quote ${quoteName} — skipping component verification`);
    }
  }
  ...
  buildRichResults({
    ...
    passed: allPass,
    extra: { bundleLineCount: preSave.length },
  });
...
async function runScenario(page, contract, branch, scenarioNumber, scenarioLabel) {
  ...
  await runQuantityIncreaseBundleNonSegment({
    page, qtc, runDir, runTs, testStartMs,
    contract, scenarioNumber, scenarioLabel, quoteName,
  });
}

test('Scenario 1: contract with 0 amendments — fresh amendment, non-segment bundle qty increase', async ({ page }) => {
...
test('Scenario 2: contract with 1 amendment — existing draft, non-segment bundle qty increase', async ({ page }) => {
...
test('Scenario 3: contract with multiple amendments — modal pick, non-segment bundle qty increase', async ({ page }) => {
