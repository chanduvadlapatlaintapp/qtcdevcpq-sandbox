...
const KIND              = 'quantityIncreaseNonSegment';
...
/**
 * Scan every `tr.product-row` and return only those with exactly 1
 * `input.qty-input` (non-segment / single-period products). Rows with 2+
 * inputs are MDQ/multi-year products and are intentionally excluded.
 *
 * A product like "DealCloud (1-year term)" renders in the same MDQ grid as a
 * multi-year product but has only one time-period column — that single input
 * qualifies as non-segment. Products not in any `tr.product-row` (legacy
 * layout) are captured via a secondary spinbutton scan.
 *
 * @param {import('@playwright/test').Page} page
 * @param {import('./utils/agenticQtcPage').AgenticQtcPage} qtc
 * @returns {Promise<Array<{rowIndex:number, input:import('@playwright/test').Locator, initial:number, rowText:string, costBefore:string}>>}
 */
async function findNonSegmentProductInputs(page, qtc) {
  /** @type {Array<{rowIndex:number, input:import('@playwright/test').Locator, initial:number, rowText:string, costBefore:string}>} */
  const results = [];
  ...
  for (let i = 0; i < rowCount; i++) {
    ...
    results.push({ rowIndex: i, input, initial: parseFloat(val) || 0, rowText, costBefore });
  }
  ...
  for (let i = 0; i < sbCount; i++) {
    ...
    results.push({ rowIndex: -1, input: sb, initial: parseFloat(val) || 0, rowText, costBefore });
  }
...
/**
 * Inner feature: find non-segment spinbuttons in the open editor, skip the
 * test if the quote has none, otherwise apply +QTY_DELTA to each, save, and
 * verify post-save quantities and metrics.
 *
 * @param {{
 ...
 * }} ctx
 */
async function runQuantityIncreaseNonSegment(ctx) {
  ...
  const nonSegmentLines = await findNonSegmentProductInputs(page, qtc);

  if (nonSegmentLines.length === 0) {
  ...
  console.log(`[Scenario ${scenarioNumber}] Found ${nonSegmentLines.length} non-segment product(s) in quote ${quoteName}`);

  const metricsBeforeSave = await captureMetrics(page);

  /** @type {Array<{rowIndex:number, input:import('@playwright/test').Locator, initial:number, filled:number, rowText:string, costBefore:string}>} */
  const preSave = nonSegmentLines.map((/** @type {any} */ l) => ({ ...l, filled: 0 }));

  for (let i = 0; i < preSave.length; i++) {
    const { input } = preSave[i];
    const current = parseFloat(await input.inputValue().catch(() => '0')) || 0;
  ...
  for (const { input } of preSave) {
    const val = await input.inputValue().catch(() => '0');
    postSave.push(parseFloat(val) || 0);
    costAfter.push(await qtc.readLineCost(input));
  }
  ...
  if (quoteName) {
    ...
    // Only fetch QuoteLines that have no SegmentIndex — these correspond to
    // non-segment (plain) products. Segment lines always have a SegmentIndex.
  ...
  if (dbLines.length > 0) {
    ...
    if (preSave.length !== dbLines.length) {
      dbAnomalies.push({ type: 'LINE COUNT MISMATCH', severity: 'HIGH',
        detail: `UI ${preSave.length} non-segment spinbuttons; DB ${dbLines.length} non-segment QuoteLines.` });
    }
  ...
  buildRichResults({
    ...
    passed: allPass,
    extra: { dbHeader, dbError, nonSegmentLineCount: preSave.length },
  });

  expect(allQtyPass, 'All non-segment line quantity assertions should pass').toBe(true);
}
...
async function runScenario(page, contract, branch, scenarioNumber, scenarioLabel) {
  ...
  await runQuantityIncreaseNonSegment({
    page, qtc, runDir, runTs, testStartMs,
    contract, scenarioNumber, scenarioLabel, quoteName,
  });
}

test('Scenario 1: contract with 0 amendments — creates new amendment, then non-segment qty increase', async ({ page }) => {
...
test('Scenario 2: contract with 1 amendment — opens existing amendment, then non-segment qty increase', async ({ page }) => {
...
test('Scenario 3: contract with multiple amendments — picks from modal, then non-segment qty increase', async ({ page }) => {
