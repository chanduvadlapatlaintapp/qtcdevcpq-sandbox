 ...
 *  Delete actions are hidden (and blocked server-side) when the quote is in Contract Executed
 *  status. Deletion is permitted for all other statuses, including Pending Closure. (BIZ-81572)
 *
...
import QUOTE_STAGE from '@salesforce/schema/SBQQ__Quote__c.Quote_Stage__c';
...
/** Quote stage that permanently locks file deletion */
const CONTRACT_EXECUTED_STAGE = 'Contract Executed';

// Row actions for ContentVersion files — full set
const CV_ACTIONS = [
...
// Row actions for ContentVersion files — no delete (Contract Executed quotes)
const CV_ACTIONS_NO_DELETE = [
    { label: 'View File Details', name: 'view', iconName: 'utility:preview' },
    { label: 'Edit', name: 'edit', iconName: 'utility:edit' }
];
...
export default class QuoteFileList extends NavigationMixin(LightningElement) {
    ...
    /** Quote stage resolved from the wired record — drives row action visibility */
    _quoteStage;
    ...
    /**
     * @description Wires the Quote's stage field so the component can suppress the Delete row
     *              action for Contract Executed quotes without a server round-trip.
     *              Also triggers the initial file load once stage data is available.
     */
    @wire(getRecord, { recordId: '$recordId', fields: [QUOTE_STAGE] })
    wiredQuote({ data, error }) {
        if (data) {
            this._quoteStage = getFieldValue(data, QUOTE_STAGE);
        } else if (error) {
            // Stage unavailable — default to showing actions; server-side guard still applies.
            this._quoteStage = null;
        } else {
            // Wire is in pending state (data and error are both undefined).
            // recordId may not yet be set — skip the load to avoid a null-Id Apex call.
            return;
        }
        this.loadFiles();
    }
     ...
     *              based on the record source and the current quote stage.
     *              Delete is suppressed for Contract Executed quotes; all other stages allow it.
     */
    async loadFiles() {
        if (!this.recordId) {
        ...
        const isLocked = this._quoteStage === CONTRACT_EXECUTED_STAGE;
        ...
        try {
            ...
            this.fileData = result.map(fw => ({
                ...
                // Suppress delete for Contract Executed; show full actions for all other stages
                rowActions: fw.source === 'ContentVersion'
                    ? (isLocked ? CV_ACTIONS_NO_DELETE : CV_ACTIONS)
                    : (isLocked ? [] : ATT_ACTIONS)
            }));
        } catch (error) {
