...
export default class GuidedSelling extends NavigationMixin(LightningElement) {
    ...
    @track searchTerm = '';
    @track currentStep = 'products'; // 'products', 'ramp', 'cart', or 'osa'
    @track rampItems = []; // Cart items with additional Ramp fields
    @track osaUrl = ''; // URL for OSA
    @track isLoadingOsa = false; // Loading state for OSA
    @track osaOpened = false; // Track if OSA was opened
    ...
    get isOsaStep() {
        return this.currentStep === 'osa';
    }

    get showCartTotalAndScore() {
        return this.currentStep === 'cart' || this.currentStep === 'osa';
    }
    ...
    get isProductsCompleted() {
        return ['ramp', 'cart', 'osa'].includes(this.currentStep);
    }

    get isRampCompleted() {
        return ['cart', 'osa'].includes(this.currentStep);
    }
    ...
    get isNextDisabled() {
        ...
        if (this.currentStep === 'osa') {
            return this.isLoading || this.isLoadingOsa;
        }
        return false;
    }
    ...
    async handleNext() {
        ...
        try {
            ...
            // Move to OSA step and load Conga URL
            await this.loadOsaUrl();
            this.currentStep = 'osa';
        } else if (this.currentStep === 'osa') {
    ...
    /**
     * @description Loads the OSA Conga URL
     */
    async loadOsaUrl() {
        if (!this.quoteId) {
        ...
        this.isLoadingOsa = true;
        this.osaOpened = false;
        try {
            const congaUrl = await generateCongaURL({ quoteId: this.quoteId });
            this.osaUrl = congaUrl;
            console.log('Conga URL loaded:', this.osaUrl);
        } catch (error) {
            console.error('Error loading Conga URL:', error);
            this.showToast('Error', 'Failed to load OSA: ' + (error.body?.message || error.message || 'Unknown error'), 'error');
        } finally {
            this.isLoadingOsa = false;
        }
    }

    /**
     * @description Opens the OSA document in a new window
     */
    handleOpenOsa() {
        if (this.osaUrl) {
            window.open(this.osaUrl, '_blank');
            this.osaOpened = true;
        } else {
            this.showToast('Error', 'OSA URL is not available', 'error');
        }
    }

    /**
     * @description Retries loading the OSA URL
     */
    handleRetryOsa() {
        this.loadOsaUrl();
    }
