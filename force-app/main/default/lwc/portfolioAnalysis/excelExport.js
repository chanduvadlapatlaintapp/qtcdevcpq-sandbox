 ...
 * @param {number} revenue  - Total revenue for the segment.
 ...
 */
function annualize(revenue, startStr, endStr) {
    const months = monthsBetween(startStr, endStr);
    if (!months || months <= 0 || revenue == null) return null;
    return (revenue / months) * 12;
}
    ...
    Object.entries(groupedData)
        ...
        .forEach(([osa, subs]) => {
            subs.forEach(s => {
                ...
                const ann = annualize(contractValue, s.SBQQ__StartDate__c, s.SBQQ__EndDate__c);
