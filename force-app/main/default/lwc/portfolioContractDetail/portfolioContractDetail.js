...
export default class PortfolioContractDetail extends LightningElement {
    ...
    get periodOptions() {
        return [
            { label: 'Past', value: 'past', btnClass: this.filterPeriods.includes('past') ? 'pcd-period-btn pcd-period-active' : 'pcd-period-btn' },
            { label: 'Current', value: 'current', btnClass: this.filterPeriods.includes('current') ? 'pcd-period-btn pcd-period-active' : 'pcd-period-btn' },
            { label: 'Future', value: 'future', btnClass: this.filterPeriods.includes('future') ? 'pcd-period-btn pcd-period-active' : 'pcd-period-btn' }
        ];
    }
    ...
    /**
     * @description Returns the subset of raw subscriptions matching all active filters.
     *              Used both by the rows getter and the filterchange event.
     * @returns {Array} Filtered SBQQ__Subscription__c records.
     */
    _getFilteredRawSubscriptions() {
        if (!this.subscriptions) return [];
        let data = this.subscriptions;
        if (this.filterDeployment !== 'all') {
            data = data.filter(s => (s.SBQQ__Product__r?.Deployment__c || '\u2014') === this.filterDeployment);
        }
        if (this.filterSupport !== 'all') {
            data = data.filter(s => (s.Support_Level__c || '\u2014') === this.filterSupport);
        }
        if (this.filterPeriods.length > 0) {
            const now = new Date();
            now.setHours(0, 0, 0, 0);
            data = data.filter(s => {
                const start = s.SBQQ__SegmentStartDate__c ? new Date(s.SBQQ__SegmentStartDate__c) : null;
                const end = s.SBQQ__SegmentEndDate__c ? new Date(s.SBQQ__SegmentEndDate__c) : null;
                if (!start || !end) return false;
                return this.filterPeriods.some(p => {
                    if (p === 'current') return start <= now && end >= now;
                    if (p === 'past') return end < now;
                    if (p === 'future') return start > now;
                    return false;
                });
            });
        }
    ...
    _fireFilterChange() {
        this.dispatchEvent(new CustomEvent('filterchange', {
            detail: { subscriptions: this._getFilteredRawSubscriptions() }
        }));
    }

    // ========== Getters — Processed Data ==========
    get rows() {
        const filtered = this._getFilteredRawSubscriptions();
        ...
        let data = filtered.map(s => {
            ...
            return {
                ...
                annualized: segMonths > 0 ? (netPrice / segMonths) * qty * 12 : null,
    ...
    handlePeriodFilter(event) {
        ...
        const current = [...this.filterPeriods];
        const idx = current.indexOf(period);
        if (idx === -1) {
            current.push(period);
        } else {
            current.splice(idx, 1);
        }
        this.filterPeriods = current;
        this._fireFilterChange();
    }
