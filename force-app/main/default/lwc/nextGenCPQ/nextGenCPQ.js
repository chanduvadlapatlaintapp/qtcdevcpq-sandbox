...
export default class NextGenCPQ extends NavigationMixin(LightningElement) {
    ...
    @track showQuoteAccelerator = false; // Quote Accelerator visibility state
    ...
    // Quote Accelerator toggle
    handleToggleQuoteAccelerator() {
        this.showQuoteAccelerator = !this.showQuoteAccelerator;
    }

    get quoteAcceleratorButtonLabel() {
        return this.showQuoteAccelerator ? 'Hide Quote View' : 'Quote Accelerator';
    }
    ...
    /**
     * @description Opens the OSA document in a new window using Conga Composer
     */
    async handleOpenOsaDocument() {
        const quoteIdToUse = this.effectiveQuoteId;
        if (!quoteIdToUse) {
        ...
        try {
            this.showToast('Info', 'Generating OSA document URL...', 'info');
            const congaUrl = await generateCongaURL({ quoteId: quoteIdToUse });
            console.log('Conga URL generated:', congaUrl);
            window.open(congaUrl, '_blank');
        } catch (error) {
            console.error('Error generating Conga URL:', error);
            this.showToast('Error', 'Failed to generate OSA document URL: ' + (error.body?.message || error.message || 'Unknown error'), 'error');
        }
    }
