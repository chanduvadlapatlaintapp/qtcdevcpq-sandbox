...
/** Strip ANSI terminal escape codes so raw Playwright error text reads cleanly in Salesforce. */
function stripAnsi(str) { return str ? str.replace(ANSI_RE, '') : str; }
...
async function insertTestResults(testRunId, tests) {
    ...
    for (const t of tests) {
        ...
        if (t.errorMessage) record.Error_Message__c = stripAnsi(t.errorMessage).slice(0, 32000);
        try {
            await sfRequest('POST', '/services/data/v62.0/sobjects/Test_Result__c', record);
            inserted++;
        } catch (e) {
            failed++;
            console.warn(`[uploader] Could not insert Test_Result__c for "${t.title}": ${e.message}`);
        }
    }
    if (failed === 0) {
        console.log(`[uploader] Inserted ${inserted} Test_Result__c record(s)`);
    } else {
        console.warn(`[uploader] Test_Result__c: ${inserted} inserted, ${failed} failed — check FLS/CRUD on Test_Result__c in Salesforce`);
    }
}
...
async function patchTestRun(testRunId, fields) {
    ...
    if (fields.errorMessage) body.Error_Message__c = stripAnsi(fields.errorMessage).slice(0, 32000);
    if (fields.logOutput)    body.Log_Output__c   = stripAnsi(fields.logOutput).slice(0, 131000);
